...
#     #grid_name  = 'asb2'
    ...
    if project == 'WRF':
        ...
        tgt_crs = genCRS('+proj=aea +lat_0=40 +lon_0=-96 +lat_1=20 +lat_2=60 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs', name=grid_name)
    elif project == 'ARB':
    ...
    if tgt_geotrans is not None and tgt_size is not None:
        ...
        tgt_geotrans = (-168.0, 1./12., 0.0, 25.0, 0.0, 1./12.); tgt_size = (1392, 720)  # (x,y) map size of NRCan grid
        resampling = 'cubic_spline'
        ...
        tgt_geotrans = (-1618000, 5000, 0, 3058000, 0, 5000);  tgt_size = (22, 33)  # 5 km
...
#     dataset_kwargs['grid'] = 'snw2'; resampling = None; lwarp = False
    ...
    domain = 1; filetype = 'hydro'
    data_mode = 'avg'; src_resampling = None; time_interval = 'clim'
    ...
    if dataset == 'WRF':
        exp_folder, exp, exp_name, domain = ds_mod.getFolderNameDomain(experiment=exp_name, domains=domain,
                                                                       exps=WRF_exps, lreduce=True, mode=data_mode,
                                                                       grid=grid_name, resampling=src_resampling)
        print('{exp_name:s}_d{dom:0=2d}'.format(exp_name=exp_name, dom=domain))
        netcdf_folder = exp_folder; bc_folder = exp_folder
        ...
        netcdf_folder, netcdf_name = ds_mod.getFolderFileName(grid=grid_name, resampling=resampling,
                                                              mode=data_mode, aggregation=time_interval,
                                                              varname='{var_str:s}', **dataset_kwargs)
    else:
        if hasattr(ds_mod, 'getFolderFileName',):
            netcdf_folder, netcdf_name = ds_mod.getFolderFileName(grid=grid_name, resampling=resampling,
                                                                  mode=data_mode, aggregation=time_interval,
                                                                  varname='{var_str:s}', dataset=dataset,
                                                                  **dataset_kwargs)
        else:
            raise NotImplementedError("Without a 'getFolderFileName' function, handling of grids " +
    ...
    driver_args = dict(); m3factor = None; raster_format = None
    # modes
    if mode.lower() == 'raster2d':
        # raster output using rasterio
        if lclip is None: lclip = True  # remove negative values
        m3factor = 1000.  # divide by this (convert kg/m^2 to m^3/m^2, SI units to HGS internal)
    ...
    if time_interval.lower() == 'hourly':
        ...
        xds = ds_mod.loadTimeSeries(varlist=varlist, multi_chunks=dict(time=time_chunks), **dataset_kwargs)
    else:
    ...
    # time format
    if time_interval.lower() == 'hourly': date_fmt = '%Y%m%d%H'; date_longfmt = '%Y-%m-%dT%H'
    elif time_interval.lower() == 'daily': date_fmt = '%Y%m%d'; date_longfmt = '%Y-%m-%d'
    elif time_interval.lower() == 'monthly': date_fmt = '%Y%m'; date_longfmt = '%Y-%m'
